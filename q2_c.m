...
NumVState=0; %number of vehicle poses stored
NumLState=0; %number of landmarks initialized
AvgObsTime = 0; %average number of observations at each time step
AvgObsLM = 0; %average number of observations received by each landmark
% graph = drivebotSLAMSystem.optimizer();
% allVertices = graph.vertices();
% allEdges = graph.edges();
for i=1:length(allVertices)
    if (isa(allVertices{i},'drivebot.VehicleStateVertex'))
        NumVState = NumVState+1;
    else
        NumLState = NumLState+1;
    end
end

for i=1:length(allEdges)
    if (isa(allEdges{i},'drivebot.LandmarkRangeBearingEdge'))
        AvgObsTime = AvgObsTime + 1/NumVState;
        AvgObsLM = AvgObsLM + 1/NumLState;
    end
end
